...
void run_inference(float* tensor_input, float* tensor_output) {
...

    {
        ...
        for(int i=0; i<QGEMM1_SIZEOUT; i++) {
            ...
            for(int j=0; j<QGEMM1_SIZEIN; j++) {
                temp += tensor_qgemm1_wq[i*QGEMM1_SIZELIN+j] * (tensor_quantize1_output[j]-QGEMM1_ZX);        
            }
...

    {
        ...
        float max_x = tensor_dequantized[0];
        for(int i=1; i<QSOFTMAX1_SIZEOUT; i++) {
            if(tensor_dequantized[i] > max_x)
                max_x = tensor_dequantized[i];
        }
        ...
        for(int i=0; i<QSOFTMAX1_SIZEOUT; i++) {
            ACCESS_EXP_LUT(tensor_dequantized[i] - max_x, exps[i])
            sum_exps += exps[i];
        }
    ...
    for(int i=0; i<DEQUANTIZE1_SIZEOUT; i++) {
        int subzero = (int)tensor_qsoftmax1_output[i]-DEQUANTIZE1_Z;
        if (subzero < INT8_MIN) {
            subzero += UINT8_MAX+1;
            subzero %= UINT8_MAX+1;
        }
            ...
            subzero %= UINT8_MAX+1;
        tensor_output[i] = subzero*DEQUANTIZE1_S;
    }
